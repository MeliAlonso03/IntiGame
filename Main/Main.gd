...
var ratita_scene = preload("res://Personajes/Ratita.tscn")  # Precargar escena Ratita
...
func _ready():
	...
	spawn_gems()  # Inicialmente generar gemas
	set_pollito_timer()
	$GameTimer.start()
...
func _process(delta):
	if $GemConteiner.get_child_count() == 0:  # Si no hay gemas
		level += 1
		time_left += 5
		spawn_gems()
		spawn_ratitas()  # Generar ratitas cuando no hay gemas
...
func spawn_ratitas():
	var ratita_count = randi() % 2 + 4  # Generar 4 o 5 ratitas aleatoriamente
	for index in range(ratita_count):
		var ratita = ratita_scene.instance()  # Crear instancia de Ratita
		ratita.position = Vector2(rand_range(0, screensize.x), rand_range(0, screensize.y))
		$GemConteiner.add_child(ratita)
...
func _on_PollitoTimer_timeout():
	...
	pollito.position = Vector2(rand_range(25, 700), rand_range(25, 430))
